...
## CDC 審計端口(FLT Statement/Port 模式)

**定案架構(2026-07-14,用戶 + GPT 5.6 外審,規範文件 `docs/00_READ_FIRST_ARCHITECTURE_CORRECTION.md` 為準)**:

**最終項目定理與審計目標 = 無條件 `cycle_double_cover : AffineCDC.Statement.CDCStatement`**
(端到端 machine-checked)。這**不是**論文-only corollary——先前把它降為 corollary 是漂移,已撤回
(見 `docs/navigation-2026-07-14.md` 頂部 SUPERSEDED 標記)。`cubic_flow_cdc` 只是**中間 macro-Port
定理**(cubic + F₂³-flow ⟹ CDC),有用但不取代無條件目標。

### 兩包架構

| 包 | 內容 | 依賴政策 | 狀態 |
|---|---|---|---|
| **A. AffineCDC 核心** | T1→gauge/coker→Fano→dart cover→port→**`cubic_flow_cdc` ✅** | **完全自圓,永不依賴 cdc-lean** | ✅ **A 完成(2026-07-14)** |
| **B. 隔離整合包** | bridgeless→cubic reduction→Γ-flow→**AffineCDC cubic-flow cover**→`cycle_double_cover : CDCStatement` | 自帶 lakefile;可依賴已形式化外層;pin 衝突在包內解決;**非核心依賴** | ⬜ P1 完成並報告後開始 |

**B 的鐵律**(file 00 §4):可重用外層 reduction,但**禁止**直接調用舊的完整 CDC 終定理
(`exact CDCLean.cycleDoubleCover_of_bridgeless …` 或等價),必須把舊 cubic cover engine 替換為
AffineCDC 構造,並提供 proof-path audit 證明新 affine 核心確被使用。

**審計**:(i) Statement 語義 `#print CDCStatement`/`IsCycle`/…;(ii) **最終** `#check cycle_double_cover`
+ `#print axioms cycle_double_cover` + proof-path 依賴圖(明示調用 AffineCDC Port 定理、未調用舊終定理)。
另有中間 `#check cubic_flow_cdc` + axiom audit。

| 層 | 檔案 | 狀態 |
|---|---|---|
| 審計面(標準無條件 CDC,只 Mathlib、只標準詞彙,cut-based `Bridgeless`) | `Statement.lean` | ✅ 編譯 |
| 核心 endpoint(構造層 + `CubicFlowCDCStatement` + `graphDartFlow` + **`cubic_flow_cdc` ✅**) | `Port.lean` | ✅ **完成(2026-07-14),零 sorry** |
| 圈分解 | `Decompose.lean` | ✅ |

**P1 施工**(`cubic_flow_cdc`,全依賴本庫核心,不 import cdc-lean):σ-閉 dart supports → 投影 edge supports
→ 證 edge membership ⟺ 任一 endpoint dart membership → unique-partner/0-or-2 證 edge support `IsEven`
→ `exists_cycle_decomposition` → 合併 Γ-indexed → exact-two dart coverage 證每邊 multiplicity=2 → 打包 + axiom audit。
**✅ P1 完成(2026-07-14)**:`cubic_flow_cdc : CubicFlowCDCStatement` 組裝完畢,零 sorry,Port 編譯通過。
鏈:σ-閉 dart supports 投影 `edgeSupp=(·.1.1)''M s` → `hEdart`(edge ⟺ endpoint dart,經 σ-閉)
→ `hSu_even`(頂點支集 darts 由 unique-partner 為 {0,2},偶)→ `hEven`(邊集偶,edgeOf 單射)
→ `exists_cycle_decomposition'` → `Finset.univ.sum 𝒟`(`mem_finset_sum`/`card_filter_sum`)
→ `hEdgeCov`(每邊恰兩支集,由 `hcov2`)→ 每邊 multiplicity=2。**依規範:報告後才開始 P2。**

**Comparison 數學支柱 = T1 classification**:T1 分類所有局部偶族,故原稿任何局部偶族自動是某 `Φ_W(m)`。
`slotFamily_eq_localFamily`(字典 `m⁰ = t+f(b)`)= T1 worked example / 論文註腳,**非**自圓支柱、非引外庫理由。

**Loops/有限性**(P3,不阻塞 P1):`Loopless` 源於 `IsEven` incidence 計數約定(勿稱「loops 無 cover」);
`[Finite α/β]` 是 ambient 約定,日後可改 `V(G).Finite ∧ E(G).Finite`。任何 Statement 改動須同步調整最終
定理型別,**不得**把目標弱化為 cubic/flow 命題。

**新數學**(`docs/02_MATHEMATICAL_UPDATE.md`,不阻塞 P1):Schur-code `B_f=(C(G)*F_f)^⊥`、Fano Hodge
自對偶 `ker δ_f ≅ Stress(f)`、exterior defect 的 codim≤1 sharpness + rank-4 K₃,₃ 反例、gauge–embedding
對應、rank-2 counting `2^{n/2−1}`(疑證 Hušek–Šámal 下界)、Petersen/cube 計算檔。列論文計劃,完成 CDC 鏈後再選低成本 Lean 化。

導航/挖礦(未來 Fable 5):`docs/navigation-2026-07-14.md`(架構部分已 SUPERSEDED,礦脈條目仍有效,但以 `02_MATHEMATICAL_UPDATE.md` 為準)。
