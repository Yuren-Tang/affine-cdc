...
**目標**:圖論專家在獨立 audit 裡快速核驗「聲稱的 CDC 就是**標準無條件** CDC、只依賴 Mathlib」。

### ⚠ 範圍修正(2026-07-14,用戶 + GPT 5.6 外審)

先前把 `CDCStatement` 定為條件版(cubic + flow ⟹ CDC)是**範圍漂移**——`Cubic`/`NowhereZeroFlow`
是**證明介面**詞彙,不是 CDC 原命題。審計面出現它們正是用戶擔心的「偷梁換柱」。**已修正**:
`Statement.lean` 現為標準無條件 CDC。關鍵事實(先前誤判):`openai/cdc-lean` **已完整形式化**
外層化約(bridgeless→cubic expansion→Jaeger–Kilpatrick 8-flow→cover projection),故無條件
CDC **可達**——把其 `cubic_even_double_cover` 替換為本庫 `DartFlow` 構造即可。

### 三層架構

| 層 | 檔案 | 內容 | 狀態 |
|---|---|---|---|
| **A 審計面** | `Statement.lean` | 只 import Mathlib、**只標準詞彙**(`IsEven`/`IsCycle`/`IsCycleDoubleCover`/`Loopless`/`CrossesCut`/`cutSet`/`Bridgeless`/`CDCStatement`)。`CDCStatement` = 「有限 loopless bridgeless Mathlib 多重圖 ⟹ ∃ CDC」——無 cubic/flow/affine。cut-based `Bridgeless`(對接 cdc-lean)。 | ✅ 編譯 |
| **B 中間端口** | `Port.lean` | `Cubic`/`NowhereZeroFlow`(**移出審計面**)+ `CubicFlowCDCStatement`(條件版,改名);`graphDartFlow`(完整 Graph→DartFlow 橋,零 sorry);圈分解 `Decompose.lean`。**剩餘**:組裝 `cubic_flow_cdc : CubicFlowCDCStatement`(edge-set 偶性 → decompose → 每邊兩次)。 | 🟡 橋完成、組裝待做 |
| **C 完整整合** | (隔離 package,待建) | 重用 cdc-lean 外層(pin `9a9483…`),以 `AffineCDC.affine_cubic_even_double_cover` 替換其 `cubic_even_double_cover`,沿原 `Main.lean` 骨架(rotation system→cubic expansion→JK flow→project→decompose)證 `cycle_double_cover : CDCStatement`;Mathlib Graph ↔ `CDCLean.FiniteGraph` adapter。 | ⬜ 待做(大工程) |

`AuditStatement.lean`:`#print` 標準定義 + `#check cycle_double_cover : CDCStatement` + `#print axioms`(期望 `propext, Classical.choice, Quot.sound`)+ 依賴審計(明示 proof path 用本庫 affine 核心)。

**施工順序**:P0 語義邊界(✅ Statement 改寫、cubic/flow 移出、改名)→ P1 完成 `cubic_flow_cdc`(本庫獨立可證,證明核心給出標準 cycles)→ P2 完整整合(重用 cdc-lean 外層)。P0–P2 期間暫停挖礦。
