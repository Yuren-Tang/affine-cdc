...
| 包 | 內容 | 依賴政策 | 狀態 |
|---|---|---|---|
| **A. AffineCDC 核心** | T1→gauge/coker→Fano→dart cover→Graph-DartFlow port→`cubic_flow_cdc` | **完全自圓,永不依賴 cdc-lean**,保持現 Mathlib pin | 🟡 橋完成、`cubic_flow_cdc` 組裝中 |
...
| 核心 endpoint(`Cubic`/`NowhereZeroFlow` 構造層 + `CubicFlowCDCStatement` + `graphDartFlow` 完整橋) | `Port.lean` | 🟡 橋✅、`cubic_flow_cdc` 組裝中 |
| 圈分解 | `Decompose.lean` | ✅ |
