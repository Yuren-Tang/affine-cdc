...
- **Comparison 最終定奪:完全自圓**——(i) 庫終點 = F2 + F3(全局偶族標籤黏合,
  自有語言);(ii) **不對齊 OpenAI 庫任何表述包括終定理**(哪怕最小對齊也須引入
  其槽位圖編碼);(iii) CDC 圈抽取(Lemma 2.1)= 論文一段引用,庫不做;
  (iv) 等價重構的形式承載 = 內化引理 `slotFamily_eq_localFamily`(已完成):
  原稿槽位構造 = `Φ(t + f(b))`,顯式字典 **m⁰ = t + 中間槽位流值**。
...
時機:第一輪 = 七條審定後、T6/T7 Lean 之前(現在);第二輪 = 全部封閉後、論文前。
