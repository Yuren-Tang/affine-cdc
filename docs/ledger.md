...
**Port 已完成**:`vertex_facts`(頂點資料打包)、`graphDartFlow`(**完整 Graph→DartFlow 橋**,含 σ 對合/invol/no_fix、頂點平面、cubic 雙射,全欄 discharged 零 sorry)。

**Port 剩餘 = 最終組裝**(`theorem cdc : CDCStatement`,精確計劃):
1. 實例:`Fintype/DecidableEq (Dartt G)`(β×α 子型,Finite→經 classical)、`(Vtx G)` 同;
   `hcodim : ∀ u, finrank (Γ ⧸ vertexPlane u) = 1` = `finrank_fin_fun`(Γ=3)
   − `IsPlane.finrank_eq_two`(平面=2)經 `finrank_quotient_add_finrank`。
2. `obtain ⟨M, hσ, hcov2, huniq, hrot⟩ := (graphDartFlow ...).exists_cycle_double_cover hcodim`。
3. 邊集 `Ĉ s := (·.1.1) '' M s`(darts 投影到邊)。σ-閉 ⟹ `e ∈ Ĉ s ↔ (e,u) 的 dart ∈ M s`。
4. **偶性** `IsEven G (Ĉ s)`:頂點 u 處 `Ĉ s ∩ incidenceSet u ≃ {支集 darts at u}`,
   後者由 `huniq`(每支集 dart 有唯一同頂點伙伴 ≠ 自身)= 無不動點對合 ⟹ 偶基數。
   **需輔助引理**「有限集上無不動點對合 ⟹ 偶 ncard」(Mathlib 無現成,~20 行)。
5. `exists_cycle_decomposition` 逐 `Ĉ s` → cycle 多重集,每邊覆蓋 [e∈Ĉ s]。
6. 合併 `𝒞 := (Finset.univ : Finset Γ).1.bind (fun s => decomp(Ĉ s))`;
   每邊 e∈E(G) 覆蓋 = Σ_{s:Γ} [e∈Ĉ s] = |{s | (e,u)的dart ∈ M s}| = 2(由 `hcov2` dart 覆蓋)。
7. `IsCycleDoubleCover G 𝒞` 打包 ⟹ `theorem cdc : CDCStatement`。
8. `AuditStatement.lean`:`#print CDCStatement` + `#print axioms cdc`;CI 加審計步。

估 ~130 行;數學已無難點(全是 dart↔edge 條件翻譯 + 計數)。
